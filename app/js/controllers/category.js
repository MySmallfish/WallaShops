(function (_, S, WS) {
    ...
    WS.CategoryController = ["$timeout", "$q", "$scope", "$filter", "$location", "productService", "dailyCacheService", "safeApply", function ($timeout, $q, $scope, $filter, $location, productService, dailyCacheService, safeApply) {
        ...
        function buildSearchParameters(routeParameters) {
            ...
            if (routeParameters.searchTerm) {
                ...
                productParameters.limit = routeParameters.limit;
                if ($scope.selectedFilterValues) {
                    productParameters.filters = _.map($scope.selectedFilterValues, function (filter) {
                        return filter.value;
                    });
                }
            }
        ...
        function extractCategoryParameters() {
            ...
            if (category) {
                ...
                $scope.limit = (routeParameters.level == 1);
                routeParameters.limit = 40;
            }
        ...
        $scope.$watchCollection("selectedFilterValues", refresh);
