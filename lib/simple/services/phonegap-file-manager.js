(function (S) {
    S.PhoneGapFileManager = function ($q, phoneGap, fileUtils) {
        ...
        function find(directory, fileName, onFound, onNotFound) {

            var promise = getFile(directory, fileName, { create: false }).then(function(entry) {
                onFound(entry.fullPath);
            }, function () {
                getDirectory(directory).then(function(directoryEntry) {
                    onNotFound(directoryEntry.fullPath + "/" + fileName);
                });
            });
            return promise;
        }
